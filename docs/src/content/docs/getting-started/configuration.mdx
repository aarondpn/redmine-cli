...
Headers are only sent to the configured server, never to hosts it redirects to. Authentication headers and request-specific headers (such as an upload's `Content-Type`) always take precedence. Without a configured `User-Agent`, the CLI identifies itself as `redmine-cli/<version>`.
