...
每个子命令都需要指定项目，可通过 `--project <identifier>` 传入，或依赖当前认证配置的默认项目（参见 [`auth`](/zh-cn/commands/auth/)）。
...
| `--section`        | 要更新的章节编号（从 1 开始）。设置后仅替换该章节，页面其余部分保持不变。需要配合 `--text`。 |
| `--section-hash`   | 原始章节内容的哈希值，Redmine 用它在章节级编辑时进行冲突检测。需要配合 `--section`。 |
