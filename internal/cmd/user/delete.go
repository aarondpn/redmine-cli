...
func newCmdUserDelete(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:   "Delete a user",
		Long:    "Delete a user. Accepts a numeric ID, login, full name, or 'me'.",
		Aliases: []string{"rm"},
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !force {
				...
				if !cmdutil.ConfirmAction(f.IOStreams.In, f.IOStreams.ErrOut, msg) {
					printer.Outcome(false, output.ActionDeleted, "user", id, "Delete cancelled")
					return nil
				}
			}

			stop := printer.Spinner("Deleting user...")
			_, err = ops.DeleteUser(context.Background(), client, ops.DeleteUserInput{ID: id})
			stop()
			if err != nil {
				return err
			}
