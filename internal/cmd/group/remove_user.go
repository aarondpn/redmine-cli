...
func newCmdGroupRemoveUser(f *cmdutil.Factory) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, err = ops.RemoveGroupUser(context.Background(), client, ops.GroupUserInput{
				GroupID: groupID,
				UserID:  userID,
			})
			stop()
			...
			printer.Action(output.ActionUserRemoved, "group", groupID,
				fmt.Sprintf("Removed user %d from group %d", userID, groupID))
			return nil
		},
	}
