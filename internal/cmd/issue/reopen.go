...
func NewCmdReopen(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			stop()
			if err != nil {
				return fmt.Errorf("failed to fetch statuses: %s", cmdutil.FormatError(err))
			}
			...
			stop()
			if err != nil {
				return fmt.Errorf("failed to reopen issue %s: %s", fmt.Sprintf("#%d", id), cmdutil.FormatError(err))
			}
